...
const plugin = async (fastify: FastifyInstance, options: FastifyPluginOptions) => {
    ...
    fastify.get<{
        ...
        const users = await fastify.db.getAll();
        reply.send({
            users
        });
    });
    ...
    fastify.get<{
        ...
        const user = await fastify.db.getOne(username);
        reply.send({user});
    });
    ...
    fastify.delete<{
        ...
        const {username} = request.params;
        await fastify.db.deleteByUsername(username);
        reply.code(204);
        reply.send();
    });
    ...
    fastify.get<{
        ...
        const {username} = request.params;
        const profile = await fastify.db.getOneProfile(username);
        reply.send({profile});
    });
